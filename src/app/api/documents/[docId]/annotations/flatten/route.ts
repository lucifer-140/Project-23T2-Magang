...
export async function POST(
  ...
  if (doc.annotations.length === 0 || !doc.fileUrl) {
    return NextResponse.json({ annotatedPdfUrl: null });
  }

  const sourcePdfPath = path.join(process.cwd(), 'public', doc.fileUrl);
  ...
  for (const ann of doc.annotations) {
    ...
    if (ann.type === 'box' && ann.width != null && ann.height != null) {
      ...
      const y = toY(ann.y) - h;
      const w = (ann.width / 100) * pw;
      page.drawRectangle({ x, y, width: w, height: h, borderColor: color, borderWidth: 1.2, color: undefined, opacity: 1 });
    }

    if (ann.type === 'draw' && ann.pathData) {
      let pts: { x: number; y: number }[];
      try { pts = JSON.parse(ann.pathData); } catch { continue; }
      if (pts.length < 2) continue;
      for (let i = 0; i < pts.length - 1; i++) {
        page.drawLine({
          start: { x: toX(pts[i].x), y: toY(pts[i].y) },
          end: { x: toX(pts[i + 1].x), y: toY(pts[i + 1].y) },
          color, thickness: 1.5, lineCap: LineCapStyle.Round,
        });
      }
    }
    ...
    if (ann.type === 'sticky' && ann.content) {
      ...
      const boxH = 24;
      const padding = 4;
      page.drawRectangle({ x, y: y - boxH, width: boxW, height: boxH, color, opacity: 0.85, borderColor: color, borderWidth: 0.5 });
      const maxChars = Math.floor(boxW / 4.5);
      const text = ann.content.length > maxChars ? ann.content.slice(0, maxChars - 1) + '…' : ann.content;
      page.drawText(text, { x: x + padding, y: y - boxH + padding, size: 7, color: rgb(0.1, 0.1, 0.1), maxWidth: boxW - padding * 2 });
    }
  }
  ...
  const outFileName = `${Date.now()}_annotated_${docId}.pdf`;
